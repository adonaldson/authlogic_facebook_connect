...
Although the Facebook documentation suggests you use the :perms key to request additional
permissions, you should instead update your init_fb_connect method call:

  <%= init_fb_connect "XFBML", {:app_settings => " { permsToRequestOnConnect : 'email' }"}  %>

(source: http://stackoverflow.com/questions/3350042/extended-permissions-in-authlogic-facebooker-plugin)

=== Notes

==== before_connect hook
If you want to save some user data when connecting to facebook you can define the
(public) before_connect hook in your user model.  before_connect is called the first
